// AdminModulIcerikSayfasi — bir modülün "Genel Bakış" içeriğini BlockNote ile düzenler.
// Yapı `AdminIcerikSayfasi`'nın aynısı; sadece tablo `unite_modulleri`.
...
import type { UniteModuluRow } from '../../lib/database.types';
...
export const AdminModulIcerikSayfasi = () => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        if (!data) {
          setYuklemeHata('Modül bulunamadı.');
          setYukleniyor(false);
          return;
        }
        setModul(data as UniteModuluRow);
        setYuklemeHata(null);
        setYukleniyor(false);
      });
    return () => {
      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1 min-w-0">
              ...
              {yukleniyor ? (
                ...
                <div>
                  <div className="text-[10px] tracking-[0.3em] uppercase text-ink-mute font-bold mb-1">
                    Modül {String(modul.sira).padStart(2, '0')} · Genel Bakış İçeriği
                  </div>
          ...
          <div className="border border-line rounded-2xl bg-bg overflow-hidden">
            {!yukleniyor && modul && (
              <IcerikEditor initialContent={modul.icerik} onChange={editorDegisti} />
            )}
          </div>
        </div>
      </div>
