...
import type { Belge, Soru, SoruWithUnite, Unite } from '../types';

const UNITELER_CACHE_KEY = 'mli_uniteler_cache_v1';
...
export const uniteleriYukle = async (): Promise<UnitelerVerisi> => {
  const [unitesR, sorularR, cozumlerR] = await Promise.all([
  ...
  (sorularR.data ?? []).forEach((s) => {
    ...
    sorularByUnite[s.unite_id].push({
      id: s.id,
      baslik: s.baslik,
      zorluk: s.zorluk,
      senaryo: s.senaryo,
...
export const uniteleriCachedenOku = (): UnitelerVerisi | null => {
  try {
    ...
    if (paket.v !== 1 || !Array.isArray(paket.uniteler)) return null;
...
export const uniteleriCacheeYaz = (uniteler: Unite[]): void => {
  try {
    const paket: OnbellekPaketi = { v: 1, ts: Date.now(), uniteler };
